...
const { findAlbumsByArtist } = require('./artistDao')
...
const albumDao = {
    ...
    findAlbumInfo: (res,table) => {
        ...
        const sql = `SELECT al.album_id, al.title ,al.artist_id,
         al.band_id, al.label_id, al.yr_released,
        CASE 
    ...
    findAlbumsByArtistId: (res,table,id) =>{
        const sql = `SELECT title, album_id, yr_released FROM ${table} WHERE artist_id = ${id};` // SELECT title, album_id, yr_released FROM ${table} WHERE artist_id = ${id};
